...
for astep in args.step.split(',') :
    ...
    os.environ['IFLOW_TECH_FILES']       = proj_path+'/'+(' '+proj_path+'/').join(Foundry.get_lefs(foundry_sel)['tech'])
    os.environ['IFLOW_ILEF_FILES']       = proj_path+'/'+(' '+proj_path+'/').join(Foundry.get_lefs(foundry_sel)['std,'+track_sel]+
    ...
    if re.search('TritonRoute',Flow.get_tool(args.design)[astep]) :
        ...
        if re.search('iEDA',Flow.get_tool(args.design)[astep]) :
            if re.search('route',astep) :
                os.system(Tools.get_path(cur_tool_name)+' -flow_config '+proj_path+'/scripts/'+
                    args.design+'/iEDA_config/flow_config_rt.json'+' -script '+proj_path+'/scripts/'+
                    args.design+'/'+astep+'.'+cur_tool_name+'.tcl'+' | tee -ia '+log_file)
            else :
                os.system(Tools.get_path(cur_tool_name)+' -flow_config '+proj_path+'/scripts/'+
                    args.design+'/iEDA_config/flow_config.json'+' -script '+proj_path+'/scripts/'+
                    args.design+'/'+astep+'.'+cur_tool_name+'.tcl'+' | tee -ia '+log_file)
        else :
